...
var aquaValues = {
	...
	fgw_3:{data:['tokenfinGoods'],val:[2],areaTotal:3000},
	rmw_4:{data:['tokenRawMat'],val:[6],areaTotal:6000},
	...
	cash_6:{data:['token5000','token1000','token100'],val:[2,7,16],areaTotal:18600},
	post_7:{data:[],val:[],areaTotal:0},
	accspay_8:{data:['tokenInvoice'],val:[4000],areaTotal:0},
...
var aquaSteps = {
	step_0:{
		text:'<h3>Assemble boats, pay for services and wages, and issue invoice</h3><p>You again assemble 4 sailboats in your assembly hall. You distribute 1 unit of raw materials and wages worth 500 over the 4 assembly lines. You pay your employees and services. You then move your boats to the warehouse of finished goods. You sold 2 boats! Well done! But unfortunately, your customers do not pay cash any more. You send an <b>invoice</b> to the customers worth 4400.</p>',
		questTxt:'First move 4000 raw materials tokens from the raw materials warehouse to the assembly lines (1000 to each line).',
		feedback:'no',
		questPanel:'yes',
		stayScreen:'no',
	...
	step_1:{
		...
		questTxt:'Now move 2000 from <b>cash</b> to employees. This 2000 is for the employee’s wages.',
		feedback:'yes',
		questPanel:'yes',
		stayScreen:'no',
		...
		cashTotal:16600,	
		changeSprite:'yes',
		changeSrc:'employees_sprite_a5_1',		
		complete:false
	},
	step_2:{
		text:'<h3>Well Done!</h3><p>You successfully moved the tokens from <b>cash</b>.</p>',
		questTxt:'Move a 500 token from Employees to each of the 4 assembly lines–move 4 tokens in total. This represents the cost of the work each employee will put into the assembly of a boat.',
		feedback:'yes',
		questPanel:'yes',
		stayScreen:'no',
	...
	step_3:{
		text:'<h3>Well Done!</h3><p>You successfully moved the tokens from employees.</p>',
		questTxt:' Move 800 from <b>cash</b> to services. This pays for services for the quarter.',
		feedback:'yes',
		questPanel:'yes',
		stayScreen:'no',
		clickelem:'cashsprite_6',
	...
	step_4:{
		...
		changeSrc:'finishedgood_sprite_a17',		
		complete:false	
	},
	step_5:{
		text:'<h3>Well Done!</h3><p>You successfully moved the tokens from the assembly lines.</p>',
		questTxt:'You sold 2 boats! Well done! Move the <b>invoice</b> worth 4400 from <b>accounts receivable</b> to customers. This represents the <b>invoice</b> to the customers for the 2 boats they purchased. This sale will not be shown in the <b>cash flow statement</b> now as no cash has moved into the company.',
		feedback:'yes',
		questPanel:'yes',
		stayScreen:'no',
		...
		areaData:[0],
		areaDataTotal:0,
		areaData1:[4400,0,0,0,0],
		areaDataTotal1:0,		
		cashData:[8,5,2],
		cashTotal:15800,	
		changeSprite:'yes',
		changeSrc:'accsrec_sprite',
		changeSrc1:'customers_sprite_a17',
		complete:false		
	},
	step_6:{
		text:'<h3>Well Done!</h3><p>You successfully moved the <b>invoice</b>.</p>',
		questTxt:'Move 2 sailboat tokens from the finished goods warehouse to customers.',
		feedback:'yes',
		questPanel:'yes',
		stayScreen:'no',
		...
		changeSrc1:'customers_sprite_a17_1',	
		complete:false
	},
	step_7:{
		text:'<h3>Well Done!</h3><p>You successfully moved the tokens.</p>',
...
function showPopNext(){		
	...
	if(moveStep == 4) appendSmallBoats();
	if(moveStep == 5) {
		$('#accsrecsprite_11').css('background-image','url('+simImgPath+'accsrec_sprite_a17.png)')
		aquaValues['accsrec_11']['data'] = ['tokenInvoice'];
		aquaValues['accsrec_11']['val'] = [4400];			
	}
	if(moveStep == 6) {
		aquaValues['cust_17']['data'] = ['tokenInvoice','tokenfinGoods','token5000','token1000','token100'];
		aquaValues['cust_17']['val'] = [4400,0,0,0,0];		
		showAreaInfo($('.cust_17'));
	}	
	if(checkComplete){		
		...
		if(aquaSteps['step_'+moveStep].customCursor != 'yes')
			{
				if((getClass+'sprite_'+getNo) == aquaSteps['step_'+moveStep].clickelem){
					...
					if(aquaSteps['step_'+moveStep].changeSprite == 'yes' && moveStep>4){
...
function finalBoatAnimation(){	
	...
	setAnimator.play({
		...
		onFrame: function(){
			...
			if((setAnimator.currentFrame() == 20) && boatStart < 2){
				boatStart++;
				finalBoatAnimation();
			}
		},
