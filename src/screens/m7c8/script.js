...
var feedback_text=["Refer to the Year 1 <b>balance sheet</b> to find the figures you need.","Good try! Select the 'Show Me' button to reveal the correct answers and enter the numbers accordingly.","You have correctly calculated the Year 1 debt repayment capacity.<br/><span><i>Select the forward arrow below to continue.</i></span>"]
...
var getAnswet=[];
var correctAnswer=['21000','3000'];
var cash_bal=['0.09','0.63'];
...
$(document).ready(function(){
	...
	$('.btn-text').on('click',function(){
		...
		if(targetId==0)
		{
			...
			$('.contentStep').html('Input the correct figures so Aquasail’s debt repayment for Year 1 can be calculated. The figures you need are in the Year 1 <b>balance sheet</b>. Aquasail generated an operational cash flow of 1900 in Year 1. ');
	...
	$('.submit').on('click',function(){
		if($('.question1').css('display')=='none'||!$('.question').hasClass('question1')){
			$('.workbook-input').each(function(i,k){
				...
				var FDC = Number(1900)/Number($(".workbook-input").eq(0).val());
				var CFDC = Number(1900)/Number($(".workbook-input").eq(1).val());
				$('.cash_statement').find('.autoFill').eq(0).text(isNaN(FDC)?FDC=0:FDC.toFixed(2));
				$('.cash_statement').find('.autoFill').eq(1).text(isNaN(CFDC)?CFDC=0:CFDC.toFixed(2));
		}
		else {
