...
var aquaSteps = {
	step_0:{
		text:'<h3>Pay for Raw Materials</h3><p>In Quarter 2, Aquasail wants to assemble 4 sailboats, but also wants to keep some raw materials in inventory for unforeseen events. Aquasail now orders 6 units of 1000 raw materials. </p>',
		questTxt:'First move 6000 from <b>cash</b> to the suppliers. This will pay for the raw materials you need to build the boats.',
		feedback:'no',
		questPanel:'yes',
		stayScreen:'no',
		...
		cashTotal:17200,	
		changeSprite:'yes',
		changeSrc:'suppliers_sprite_a8',
		complete:false	
	},
	step_1:{
		text:'<h3>Well Done!</h3><p>You successfully moved the tokens from <b>cash</b>.</p>',
		questTxt:'Now move 6 raw materials tokens from suppliers (each worth 1000) to the raw materials warehouse. This represents the raw materials you have just purchased from the suppliers.',
		feedback:'yes',
		questPanel:'yes',
		stayScreen:'no',
	...
	step_2:{
		text:'<h3>Well Done!</h3><p>You successfully moved the tokens from suppliers.</p>',
		questTxt:'',
		feedback:'yes',
		questPanel:'no',
		stayScreen:'no',
		...
		if(aquaSteps['step_'+moveStep].customCursor == 'yes'){			
			if((getClass+'sprite_'+getNo) == aquaSteps['step_'+moveStep].clickelem){				
				...
				if(aquaSteps['step_'+moveStep].dualFlip == 'yes'){
					aquaSteps['step_'+moveStep].customCursor = 'yes';					
