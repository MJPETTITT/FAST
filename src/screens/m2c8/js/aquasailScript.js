...
var aquaValues = {
	...
	cash_6:{data:['token5000','token1000','token100'],val:[4,5,10],areaTotal:26000},
...
var aquaSteps = {
	step_0:{
		text:'<h3>PAY FOR LAND, BUILDINGS AND MACHINERY</h3><p>You use the <b>cash</b> raised to invest in land, buildings and machinery. You have to pay 4000 for the land, 8000 for the buildings and 4800 for the machinery. Everything has to be paid in <b>cash</b>.</p>',
		questTxt:' First move 12000 for the land and buildings from <b>cash</b> to the commercial property agent. ',
		feedback:'no',
		questPanel:'yes',
		stayScreen:'no',
	...
	step_1:{
		...
		questTxt:' Now move 4800 for machinery from <b>cash</b> to suppliers.',
		feedback:'yes',
		questPanel:'yes',
		stayScreen:'no',
		clickelem:'cashsprite_6',
...
var showsprites_1 = ["fgwsprite_3","rmwsprite_4","bankdtsprite_9","accspaysprite_8","servicessprite_15","custsprite_17"];
var showSvgs_1 = ['fgw','rmw','accspay','bankdt','services','cust'];
var showsprites_2 = ["empssprite_2","asslines2sprite_5","asslines4sprite_5","asslines3sprite_5","asslines1sprite_5"];
var showSvgs_2 = ['emps','asslines1_5','asslines2_5','asslines3_5','asslines4_5',];
...
var filterLegend_1 = ['Employees','Assembly Lines'];
var filterLegend_2 = [''];
...
function showPopNext(){
	...
	$.each(window['showsprites_'+moveStep], function(ind, elem){
		$('#'+elem).css('display','block');
	});
	$.each(window['showSvgs_'+moveStep], function(ind, elem){
		$('#'+elem).css('display','block');
	});
	var showhide = (moveStep < 2)?'block':'none';
	$('.bg1overlay').css('display',showhide);	
	if(moveStep >= 2) $('.simBase').attr('src', simImgPath+'canvasBase_full.jpg');
	var showdisplay = ($('.legend').css('display') == 'none')?'none':'block';
	for(var i=0;i<Object.keys(aquaLegends).length;i++)
		{
			var legObj = Object.keys(aquaLegends)[i];
			var lbl = aquaLegends[legObj].txt;
			var lp = aquaLegends[legObj].lpos;
			var tp = aquaLegends[legObj].tpos; 
			$('.legendDis').append('<div class="legendAqua" data-label="'+lbl+'" style="display:'+showdisplay+';left:'+lp+'px;top:'+tp+'px;">'+lbl+'</div>');
		}
	$.each(window['filterLegend_'+moveStep], function(ind, elem){
		$('[data-label="'+elem+'"]').remove();
	});	
	var minH = parseInt($('.aquasailPop').css('min-height')) - $('.aquasilInner').height();
	$('.aquasailPop').css('min-height',minH);
}	
...
function againElemUpdate(elem){
	...
	console.log(aquaSteps['step_'+moveStep].areaData);
