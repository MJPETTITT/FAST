...
var aquaValues = {
	supls_1:{data:['tokenInvoice','tokenRawMat','token1000','token100'],val:['',4,0,0],areaTotal:4000},
	...
	cash_6:{data:['token5000','token1000','token100'],val:[4,1,0],areaTotal:21000},
...
var aquaSteps = {	
	step_0:{
		text:'<h3>Pay for Raw Materials</h3><p>In Quarter 3, Aquasail again plans to assemble 4 sailboats, and therefore buys 4 units of raw materials from its supplier. As the company now negotiated a 3-month payment delay, no cash changes hands in this transaction, but the supplier sends an <b>invoice</b>, worth 4000, that needs to be paid at the beginning of next quarter. You will move the <b>invoice</b> from suppliers to <b>accounts payable</b>. This will help you to remember your future payment obligation! This is a new <b>liability</b> for Aquasail.</p>',
		questTxt:'First move 4 raw materials tokens from suppliers (each worth 1000) to the raw materials warehouse. This represents the raw materials you have just purchased from the suppliers.',
		feedback:'no',
		questPanel:'yes',
		stayScreen:'no',
		...
		cashTotal:21000,	
		changeSprite:'yes',
		changeSrc:'suppliers_sprite_a12',
		changeSrc1:'rawmaterials_sprite_a8',	
		complete:false
	},
	step_1:{
		text:'<h3>Well Done!</h3><p>You successfully moved the tokens to raw materials.</p>',
		questTxt:'Now move the new <b>invoice</b> with 4000 on it from suppliers to <b>accounts payable</b>.',
		feedback:'yes',
		questPanel:'yes',
		stayScreen:'no',
		clickelem:'suplssprite_1',
		...
		changeSrc1:'accspay_sprite_a12',
		complete:false	
	},	
	step_2:{
		text:'<h3>Well Done!</h3><p>You successfully moved the <b>invoice</b>.<br/><br/>The <b>invoice</b> is now displayed in <b>accounts payable</b>. You do not need to update the financial documents at this point.<br/><br/>When you are ready, select the arrow to continue.</p>',
		questTxt:'',
		feedback:'yes',
		questPanel:'no',
		stayScreen:'no',
...
function showPopNext(){
	aquaValues['supls_1']['val'] = [4000,0,0,0];	
	moveStep++;	
	...
	if(checkComplete){		
		...
		$('.area-info-box-holder,#closePanel').css('display','none');
...
function finalBoatAnimation(){	
	...
	setAnimator.play({
		...
		onStop: function() {
			boatSel.stop().fadeOut(500);
		}
	});
		
	if(boatStart < 4){
		var boatInterval = window.setTimeout(function(){
			boatStart++;
			finalBoatAnimation();
		},1000);
	} 
	else{
		window.clearTimeout(boatInterval);			
	}
}
