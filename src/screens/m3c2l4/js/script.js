...
var feedback_text=["Calculate the cash balance as a result of the 800 cash outflow.","Good try! Select the 'Show Me' button to reveal the correct answers and enter the numbers accordingly.","You have correctly updated the <b>cash flow statement</b>.<br/><span><i>Select the forward arrow below to continue.</i></span>"]
...
$(document).ready(function(){
	...
	$('.btn-text').on('click',function(){
		...
		if(targetId==0)
		{
			...
			$('.contentStep').html('Update the Year 1, Quarter 2 <b>cash flow statement</b> to record the cash outflow of 800 for services, and the impact of that cash outflow on the cash balance.');
		...
		if(!$('.btn-close').hasClass('btn-icon'))
		{
			if($('.feedback-holder').find('.show').text()=='SHOW ME')
			{
				...
				/*$('.statementPop').find('.title-text').text('CASH STATEMENT: LABOUR')
				 $('.statementPop').find('.primary-text').html('<p>Now, update the if($('.contentStep').height() < 40) return;, Quarter 1 Cash Statement to record the Cash outflow of 2000 in Wages and the impact of that Cash outflow on the Cash Balance.</p>');
				$('.feedback-holder').find('.show').addClass('btn-icon').removeClass('btn-text');
				$('.feedback-holder').find('.show').text('');
