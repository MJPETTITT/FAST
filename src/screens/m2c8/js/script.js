...
$(document).ready(function(){
	...
	$('.btn-text').on('click',function(){
		...
		if(targetId==0)
		{
			...
			$('.contentStep').html('Update the <b>cash flow statement</b> to record the cash outflow. land: -4000, buildings -8000, and machinery -4800. Cash payments by Aquasail should always be recorded with a minus sign.');
		...
		else if(targetId==1)
		{
			$('.question').show();
			$('.move-box-holder').show();
			$('.contentStep').show().html('Update the <b>balance sheet</b> (on the right). Remember, land: 4000, buildings 8000, and machinery 4800. Also update the change in <b>cash</b>.');
	...
	$('.show').on('click',function(){
		...
		if(!$('.btn-close').hasClass('btn-icon'))
		{
			if($('.feedback-holder').find('.show').text()=='SHOW ME')
			{
				...
				$('.statementPop').find('.primary-text').html("<p>Now update Aquasail's <b>balance sheet</b>. The right hand side does not change: there are no new sources of funding. Remember, land: 4000, buildings: 8000, machinery: 4800.</p>");
				$('.feedback-holder').find('.show').addClass('btn-icon').removeClass('btn-text');
				$('.feedback-holder').find('.show').text('');
				$('.contentStep').hide();
