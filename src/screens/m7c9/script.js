...
var getAnswet=[];
var correctAnswer=['9000','3000'];
var cash_bal=['0.34','1.03'];
...
$(document).ready(function(){
	...
	$('.btn-text').on('click',function(){
		...
		if(targetId==0)
		{
			...
			$('.contentStep').html('Input the correct figures so Aquasail’s debt repayment for Year 2 can be calculated. The figures you need are in the Year 2 <b>balance sheet</b>. Aquasail increased its operational cash flow in Year 2 to 3100.');
	...
	$('.submit').on('click',function(){
		if($('.question1').css('display')=='none'||!$('.question').hasClass('question1')){
			$('.workbook-input').each(function(i,k){
				...
				var FDC = Number(3100)/Number($(".workbook-input").eq(0).val());
				var CFDC = Number(3100)/Number($(".workbook-input").eq(1).val());
