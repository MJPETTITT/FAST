...
var aquaSteps = {
	...
	step_1:{
		text:'<h3>Well Done!</h3><p>When you select a location, you’ll see that it flips over and reveals 1 or more token holders. These token holders may contain <b>cash</b>, raw materials or finished goods. Throughout this primer you will move <b>cash</b> or other resources into, through and out of the business. This will enable you to understand how a company’s finances work.<br/><br/>Selecting the suppliers location was just for practice. Now, let’s start the Aquasail business for real!</p>',
		questTxt:'Use the navigation arrows and the map to locate the <b>bank</b>. Then select the <b>bank</b>.',
		feedback:'yes',
		questPanel:'yes',
		stayScreen:'yes',
		clickelem:'',
	...
	step_2:{
		text:'<h3>Ready? Let’s start! </h3><p>It’s January 1. The first financial action Aquasail takes is to issue shares, representing the equity of Aquasail. Shareholders own the business. You will invest 14000 of your own savings in the company in return for all the shares of Aquasail. You invest that money on Aquasail’s newly opened bank account, which we label &#39;<b>cash</b>&#39; in the simulation. </p>',
		questTxt:'',
		feedback:'no',
		questPanel:'yes',
		stayScreen:'yes',
	...
	step_3:{
		text:'<h3>Cash</h3><p>To do this, move <b>cash</b> from the shareholders to the <b>cash</b> location in the middle of the Aquasail building. This cash location represents all the <b>cash</b> you currently have available in the business. Your current cash situation is always displayed on the dashboard at the top left of the simulation.</p>',
		questTxt:'Select the funds to pick up 14000 in money tokens. Notice that when you select a location, the <b>cash</b> or resources it contains are shown on the dashboard at the <b>top right</b> of the simulation.<br/><br/>Drag the tokens to the cash location in Aquasail and deposit them there. Notice that the cash location animates so you know where the <b>cash</b> needs to go.',
		feedback:'no',
		questPanel:'yes',
		stayScreen:'no',
	...
	step_4:{
		text:'<h3>Well Done!</h3><p>With that move you successfully moved 14000 <b>cash</b> from the shareholders to Aquasail, and at the same time mastered the main drag-and-drop functionality you will be using in the simulation!<br/><br/>Occasionally you will be asked to write yourself a note to remind yourself where the <b>cash</b> came from.</p>',
		questTxt:'To make a  note of the 14000 <b>cash</b> from shareholders, first select the post-it pad. A new window will open, asking you to type in the amount. Type in 14000 and select ‘write’. Then drag the post-it pad to the equity location. This will help you remember where the money came from.',
		feedback:'yes',
		questPanel:'yes',
		stayScreen:'no',
...
function canvasResize(){	
	...
	$('.canvasBase').stop().animate({
		  transform: 'translate('+leftPos+'px, '+topPos+'px) scale('+(scalePos)+') translate3d(0, 0, 0)'
		},400,function(){			
	});
	...
	if(parseInt(xPos) == 0){
		...
		if(selector.attr('data-forward') == 1) getAnimator.reverse();	
         getAnimator.play({
			...
			onStop: function(){
				if(getAnimator.currentFrame() >= 29){
...
var leafTreePos = [[870,840],[730,1070],[600,1270],[450,1530],[324,1760],[210,1950],[325,2190],[465,2440],[585,2670],[725,2900],[875,3170],[1365,2485]];
